    ...
    <div className="financial-dashboard">
      <div className="section-header">
        ...
        <div className="financial-summary">
          ...
          <span className="summary-item">
            💳 {formatMoney(averageOrderValue)} Avg
          </span>
        </div>
      </div>
            ...
            {financialData.dailyRevenue.slice(0, 7).reverse().map((day, index) => (
              <div key={index} className="chart-bar">
                <div 
                  ...
                  title={`${formatMoney(day.revenue || 0)} on ${new Date(day.date).toLocaleDateString(locale)}`}
                ></div>
                <div className="bar-label">
                  {new Date(day.date).toLocaleDateString(locale, { weekday: 'short' })}
                </div>
                <div className="bar-value">
                  {formatMoney(day.revenue || 0)}
                </div>
              </div>
            ))}
          </div>
                ...
                <div key={index} className="payment-method">
                  <div className="method-info">
                    ...
                    <span className="method-amount">
                      {formatMoney(method._sum.amount || 0)}
                    </span>
                  </div>
