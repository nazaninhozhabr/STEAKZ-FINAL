...
import { formatCurrency, resolveLocale } from '../../utils/formatCurrency';
...
const StatisticsCards: React.FC<StatisticsCardsProps> = ({ statistics }) => {
  ...
  const formatMoney = (amount: number) => formatCurrency(amount, settings.currency, settings.language);

  const cards = [
    {
      title: 'Total Revenue',
      value: formatMoney(statistics.revenue.total),
      subtitle: `Today: ${formatMoney(statistics.revenue.today)}`,
      icon: '💰',
      color: 'green',
      trend: {
    ...
    },
    {
      ...
      subtitle: `${formatMoney(statistics.revenue.week)} revenue`,
      icon: '📈',
      color: 'orange',
      trend: {
    ...
    },
    {
      ...
      subtitle: `${formatMoney(statistics.revenue.month)} earned`,
      icon: '🎯',
      color: 'teal',
      trend: {
        value: '+15.2%',
