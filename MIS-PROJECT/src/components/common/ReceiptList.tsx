...
interface ReceiptListProps {
  ...
  onReprint?: (receipt: ReceiptLike) => void;
  onEmail?: (receipt: ReceiptLike) => void;
}
  ...
  return (
    <div style={{ display: 'grid', gap: '16px' }}>
      {receipts.map(receipt => {
        const isReceipt = 'paymentMethod' in receipt;
        const amount = isReceipt ? receipt.total : receipt.amount;
        const method = isReceipt ? receipt.paymentMethod : receipt.method;
        const status = !isReceipt
          ? receipt.status
          : (receipt.order?.payment?.status || 'COMPLETED');
        return (
          <div key={receipt.id} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
              ...
              <span style={{
                ...
                fontSize: '12px',
                fontWeight: '500',
                backgroundColor: status === 'COMPLETED' ? '#d1fae5' : '#fee2e2',
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '8px', marginBottom: '16px' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '14px', fontWeight: '500' }}>{method.replace('_', ' ')}</div>
              </div>
              <div>
