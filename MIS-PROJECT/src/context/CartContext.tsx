...
export const CartProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const getTotalItems = () => {
    ...
    try {
      // Require explicit branch selection (customers) or assigned branch (staff)
      let targetBranchId = branchId;

      if (user.role === 'CUSTOMER') {
        targetBranchId = branchId || user.branchId;
      } else {
      ...
      if (!targetBranchId) {
        throw new Error('Please select a branch before placing your order');
      }
