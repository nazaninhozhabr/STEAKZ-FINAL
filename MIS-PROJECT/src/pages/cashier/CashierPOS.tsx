...
import { createOrder, processPayment, getMenuItems, createReceipt } from '../../services/api';
...
const CashierPOS: React.FC = () => {
  ...
  const categories = ['ALL', ...Array.from(new Set(menuItems.map(item => item.category)))];  const handlePlaceOrder = async () => {
    ...
    if (!user || !user.branchId) {
      setMessage('User not authenticated or branch not assigned.');
      return;
    }
    ...
    try {
      ...
      const orderRes = await createOrder({ 
        branchId: user.branchId, // Use cashier's branch
        items: cart, 
        deliveryAddress: customerName ? `In-store order - Customer: ${customerName}` : 'Walk-in Customer'
      });
      ...
      try {
        ...
        const receiptRes = await createReceipt(receiptData);
        console.log('Receipt created:', receiptRes.data);
      } catch (receiptError) {
        console.error('Receipt creation failed:', receiptError);
        // Don't fail the whole order if receipt creation fails
      }
      ...
      if (orderId) {
        ...
        try {
          ...
          // Create receipt after successful payment retry
          try {
            const receiptData = {
              orderId: orderId,
              subtotal: getSubtotal(),
    ...
    return `
      ...
      <body>
          ...
          ${cart.map(cartItem => {
            ...
            return `
              <div class="item">
                ...
                <span>${formatCurrency(itemTotal, settings.currency, settings.language)}</span>
              </div>
            `;
          }).join('')}
        </div>
        ...
        <div class="total-section">
          <div class="item">
            <span>Subtotal:</span>
            <span>${formatCurrency(getSubtotal(), settings.currency, settings.language)}</span>
          </div>
          ${discountPercent > 0 ? `
            <div class="item">
              <span>Discount (${discountPercent}%):</span>
              <span>-${formatCurrency(getDiscountAmount(), settings.currency, settings.language)}</span>
            </div>
          ` : ''}
          <div class="item">
            <span>Tax (${taxPercent}%):</span>
            <span>${formatCurrency(getTaxAmount(), settings.currency, settings.language)}</span>
          </div>
          <div class="item total">
            <span>TOTAL:</span>
            <span>${formatCurrency(getFinalTotal(), settings.currency, settings.language)}</span>
          </div>
        </div>
  ...
  return (    <div className={isDarkMode ? 'dark' : ''} style={{ minHeight: '100vh', backgroundColor: isDarkMode ? '#18181b' : '#f9fafb', padding: '24px' }}>
                  ...
                  <>
                          ...
                          <div key={cartItem.menuItemId} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid #f3f4f6' }}>
                            <div style={{ flex: 1 }}>
                              ...
                              <div style={{ fontSize: '12px', color: '#6b7280' }}>
                                ${formatCurrency(menuItem.price, settings.currency, settings.language)} each
                              </div>
                            </div>
                    ...
                    <div style={{ padding: '16px 0', borderTop: '2px solid #e5e7eb' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                        ...
                        <span style={{ fontSize: '14px', fontWeight: '500' }}>{formatCurrency(getSubtotal(), settings.currency, settings.language)}</span>
                      </div>
                      
                      {discountPercent > 0 && (
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                          <span style={{ fontSize: '14px', color: '#6b7280' }}>Discount ({discountPercent}%):</span>
                          <span style={{ fontSize: '14px', fontWeight: '500', color: '#ef4444' }}>-{formatCurrency(getDiscountAmount(), settings.currency, settings.language)}</span>
                        </div>
                      )}
                      
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                        <span style={{ fontSize: '14px', color: '#6b7280' }}>Tax ({taxPercent}%):</span>
                        <span style={{ fontSize: '14px', fontWeight: '500' }}>{formatCurrency(getTaxAmount(), settings.currency, settings.language)}</span>
                      </div>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', paddingTop: '8px', borderTop: '1px solid #e5e7eb' }}>
                        ...
                        <span style={{ fontSize: '18px', fontWeight: 'bold', color: '#059669' }}>
                          {formatCurrency(getFinalTotal(), settings.currency, settings.language)}
                        </span>
                      </div>
