...
import { Payment, OrderWithDetails, Receipt } from '../../types';
...
const CashierReceipts: React.FC = () => {
  ...
  const handleReprint = (receipt: ReceiptLike) => {
    ...
    const payment: Payment | undefined = 'amount' in receipt ? receipt : order.payment;
    if (!payment) {
      setMessage('Payment data not found for reprint');
      setTimeout(() => setMessage(''), 3000);
      return;
    }
  ...
  const handleEmail = async (receipt: ReceiptLike) => {
    // Find the associated order
    const order = orders.find(o => o.payment?.id === receipt.id);
    if (!order) {
    ...
    const payment: Payment | undefined = 'amount' in receipt ? receipt : order.payment;
    if (!payment) {
      setMessage('Payment data not found for email');
      setTimeout(() => setMessage(''), 3000);
      return;
    }

    setEmailModal({ show: true, receipt: payment, order });
  };
        ...
        <div className="receipts-list">
          ...
          {loading ? (
            ...
            <ReceiptList
              ...
              onEmail={(receipt) => {
                const order = orders.find(o => o.payment?.id === receipt.id) || null;
