...
// Public menu items (no auth) - requires branchId
export const getMenuItemsPublic = (branchId: number) =>
  axios.get<MenuItemWithIngredients[]>(`${API_URL}/api/menu-items/public`, { params: { branchId } });
...
export const createReceipt = (data: {
  ...
  customerName?: string;
}) => api.post<any>('/api/receipts', data);
...
export const getReceipts = (params?: {
  ...
  endDate?: string;
  paymentMethod?: string;
}) => api.get<any[]>('/api/receipts', { params });

export const getReceiptById = (id: number) =>
  api.get<any>(`/api/receipts/${id}`);

export const getReceiptStatistics = (params?: {
  branchId?: number;
  startDate?: string;
  endDate?: string;
}) => api.get<any>('/api/receipts/statistics', { params });
