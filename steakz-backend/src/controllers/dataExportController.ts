...
export async function startExport(req: Request, res: Response) {
  try {
    ...
    if (user.role === 'BRANCH_MANAGER' && !branchId) {
      return res.status(400).json({ message: 'Branch manager must be assigned to a branch' });
    }

    process.nextTick(() => processExportJob(jobId, user.id, dataTypes, dateRange, customFields, branchId));
...
async function processExportJob(jobId: string, userId: number, dataTypes: string[], dateRange: any, customFields: string[], branchId?: number) {
  try {
    let allData: any[] = [];
    let headers: string[] = [];
    let progress = 0;
    for (const [i, type] of dataTypes.entries()) {
      let data: any[] = [];
      if (type === 'sales') {
        data = await exportSalesData(dateRange?.start, dateRange?.end, customFields, branchId);
        headers = generateCSVHeaders('sales', customFields);
      } else if (type === 'inventory') {
        data = await exportInventoryData(customFields, branchId);
...
async function exportSalesData(startDate?: string, endDate?: string, _fields: string[] = [], branchId?: number) {
...
async function exportInventoryData(_fields: string[] = [], branchId?: number) {
  return await prisma.inventoryItem.findMany({
    where: { quantity: { gt: 0 }, branchId: branchId ?? undefined },
    select: {
      id: true,
      name: true,
      quantity: true,
